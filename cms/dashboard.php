<?php
if($_SESSION['nicolalawrence_admin']['admin_login']!=md5($_SESSION['nicolalawrence_admin']['admin_password'].session_id()))
{
	header("location:index.php");
	exit;
}
?>
<h1>Dashboard</h1>
<div class="clearboth"></div>
<div class="dashboard_paragraph">
	<p>Dear <?php echo $_SESSION['nicolalawrence_admin']['admin_fullname']; ?>,</p>
    <p>Welcome to <b><?php echo SITE_NAME; ?></b> website content management system, URL: <a href="<?php echo SITE_URL; ?>" target="_blank" class="mylink1"><?php echo SITE_URL; ?></a></p>
    <p>Do not forget to <a href="logout.php" class="mylink1">logout</a> when you have finished using the system.</p>
    <p style="margin-top:10px;">Please click the required area that you wish to manage below:</p>
</div>

<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Home</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">Manage contents for the home page.<br>
    <br>
    <a href="login.php?p_id=manage_home" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>About</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">Manage contents for the the about page. <br>
    <br>
    <a href="login.php?p_id=manage_about" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="mpright">
  <div class="maintitle" style="padding-left: 10px;"><b>Contact</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;"> Manage contents for the contact page. <br>
    <br>
    <a href="login.php?p_id=manage_contactus" class="dashboard_btn">Click Here</a> </div>
</div>

<div class="spacer20"></div>
<h1>Products</h1>
<div class="spacer20"></div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Textiles</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;"> Manage textile collections and it's products.<br>
    <br>
    <a href="login.php?p_id=manage_textiles&id=1" class="dashboard_btn">Click Here</a> </div>
</div>

<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Wallpapers</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;"> Manage wallpaper collections and it's products. <br>
    <br>
    <a href="login.php?p_id=manage_wallpapers&id=2" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="mpright">
  <div class="maintitle" style="padding-left: 10px;"><b>Decorative Pieces</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;"> Manage decorative piece and it's products.<br>
    <br>
    <a href="login.php?p_id=manage_decorative_pieces&id=3" class="dashboard_btn">Click Here</a> </div>
</div>

<div class="spacer20"></div>
<h1>The Edit</h1>
<div class="spacer20"></div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>All News</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">Manage contents for the news page. <br>
    <br>
    <a href="login.php?p_id=manage_blogs" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Resources</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;"> Manage contents for the resources page. <br>
    <br>
    <a href="login.php?p_id=manage_resources" class="dashboard_btn">Click Here</a> </div>
</div>

<div class="mpright">
  <div class="maintitle" style="padding-left: 10px;"><b>Instagram</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">Manage contents for the instagram page. <br>
    <br>
    <a href="login.php?p_id=manage_instagram" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="spacer20"></div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Gallery</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;"> Manage contents for the gallery page. <br>
    <br>
    <a href="login.php?p_id=manage_gallery" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="spacer20"></div>
<h1>Terms & conditions and Sales Order</h1>
<div class="spacer20"></div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Terms & conditions</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">Manage contents for the terms & conditions page. <br>
    <br>
    <a href="login.php?p_id=manage_legal&page=1" class="dashboard_btn">Click Here</a> </div>
</div>


<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Orders</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">View sales orders. <br>
    <br>
    <a href="login.php?p_id=manage_sales" class="dashboard_btn">Click Here</a> </div>
</div>
<div class="spacer20"></div>
<h1>Website Status</h1>
<div class="spacer20"></div>
<div class="mpleft">
  <div class="maintitle" style="padding-left: 10px;"><b>Website Status</b></div>
  <div class="maincont" style="padding: 10px; text-align: left;">Change website Status. <br>
    <br>
    <a href="ajax/change_website_status.php" class="dashboard_btn change_status fancybox.ajax">Click Here</a> </div>
</div>
	<script>
	$(function(){
		$('.change_status').fancybox();	
	});
	</script>